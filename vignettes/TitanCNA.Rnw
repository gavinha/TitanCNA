...
<<<<<<< HEAD
<<<<<<< HEAD
  \item genotypeParams: Parameters for copy number and allelic ratios genotype states
...
<<<<<<< HEAD
<<<<<<< HEAD
registerDoMC(cores = 10) #use 10 cores on a single machine
=======
...
<<<<<<< HEAD
<<<<<<< HEAD
plotCNlogRByChr(results, chr = 2, ploidy = ploidy, ylim = c(-2, 2), cex = 0.25, 
                xlab = "", main = "Chr 2")
=======
=======
>>>>>>> master
...
<<<<<<< HEAD
<<<<<<< HEAD
plotSubcloneProfiles(results, chr = 2, cex = 2, spacing = 6, main = "Chr 2")
=======
