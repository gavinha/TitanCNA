...
Users can specify the desired chromosome naming convention by using the \verb|genomeStyle| argument. Using \verb|NCBI| will use chromosome names such as 1, 2, 3, ..., X, while using \verb|UCSC| will use names such as chr1, chr2, chr3, ..., chrX. Users can further change chromosome naming convention after by invoking \verb|setGenomeStyle|. Note that it does not matter what the chromosome convention was originally; using \verb|genomeStyle| argument and \verb|setGenomeStyle| function will return the specified convention.

<<>>= 
data$chr <- setGenomeStyle(data$chr, genomeStyle = "UCSC")
head(data$chr)
data$chr <- setGenomeStyle(data$chr, genomeStyle = "NBCI")
head(data$chr)
@
...
Including the loaded \verb|data| from the previous step will help inform the baseline allelic ratio parameters; this is recommended when using \verb|symmetric| genotypes.
...
\begin{itemize}
  ...
  \item \verb|sParams|: Parameters for modeling subclonality: clonal clusters and cellular prevalence
...
The format of this output file and data.frame is the following:
\begin{enumerate}
  \item \verb|Chr|
  \item \verb|Position|
...
New in version 1.8.1, users can post-process the results to exclude clonal clusters that have fewer genomic alterations.  Users can specify the minimum proportion of the genome altered for any subclonal cluster using \verb|proportionThreshold| and the minimum proportion of clonal events for the clonal (highest cellular prevalence) cluster using \verb|proportionThresholdClonal|.  The function returns the post-processed \verb|convergeParams| and \verb|results| objects in a 2 element list.
...
New in version 1.10.1, users can now directly generate segments and output these to files.  There are two possible outputs to files: 1) Segments with detailed information, and 2) Segments prepared for loading into IGV.  These filenames can be specified in arguments \verb|filename| and \verb|igvfilename|
...
The definitions of the headers for the segments are the following:
\begin{enumerate}
	...
	\item \verb|TITAN_state|, \verb|TITAN_call|, \verb|Copy_Number|: Same as defined above
	\item \verb|MinorCN|: Copy number of minor allele
	\item \verb|MajorCN|: Copy number of major allele
	\item \verb|Clonal_Cluster|, \verb|Cellular_Frequency|: Same as defined above
\end{enumerate}
