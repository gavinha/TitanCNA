...
sample = jointpysam.bam.BamFile(bam_file)
reference = jointpysam.fasta.FastaFile(ref_file)
## print header ##
...
for chrom in positions:
    ...
    for position_data in positions[chrom]:
        ...
        if qual >= vcf_quality and qual != None:
            _p = sample.get_pileup_iterator(chrom,position,position + 1)
            pileup = None
            for p in _p:
            ...
            if pileup is None:
                result += "\t" + ref_base + "\t0\t" + nref_base + "\t0" + "\t" + str(qual)
                #print result
                continue
        
            ref_count = 0
            depth = 0
            bases = filter((lambda base: base['base_qual'] >= base_quality and base['map_qual'] >= map_quality), pileup.base_info)
